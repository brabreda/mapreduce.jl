...
const KA_V2_file = path*"/../benchmarks/CUDA/KA_scalar_v2.csv"
...
min_times_KA_V2 = combine(groupby(KA_V2_scalar, [:N, :type, :op]), "times" => minimum => :min_time)
min_times_CUB = combine(groupby(CUB, [:N, :type, :op]), "times" => minimum => :min_time)


min_times_CUDA[!, :name] .= "CUDA.jl "
min_times_KA_V2[!, :name] .= "Vendor neutral "
min_times_CUB[!, :name] .= "CUB "


min_times_CUDA[!, :impl] .= "CUDA.jl"
min_times_KA_V2[!, :impl] .= "Vendor neutral"
